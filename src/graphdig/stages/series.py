...
def _x_keys(cal, n: int) -> list[str]:
    x = cal.x_axis
    ...
    if x.kind == "numeric" and x.start and x.end:
        try:
            xs = np.linspace(float(x.start), float(x.end), n)
...
def _unit_for(ctx: Context, cal, x_key: str) -> Unit:
...
def run(ctx: Context) -> None:
    ...
    for tile in tiles_art.tiles:
        ...
        if baseline_art is not None:
            pb = baseline_art.panels.get(pid)
            if pb and pb.line_visible and len(pb.points) >= 2 and pb.beta_px is not None:
                base_pts = np.array([[p.x, p.y] for p in pb.points])
        ...
        if gaps:
            ctx.add_flag("series", f"{len(gaps)} empty slice(s): {', '.join(gaps[:5])}"
                         + ("..." if len(gaps) > 5 else ""), panel_id=pid, severity="warning")
