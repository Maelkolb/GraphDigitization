# Run report: 20260702-221651-290053_tif_M11

- profile: **danube**
- graphdig: 0.1.0
- input: `pages/290053_M11.png` (686x424)

## Panels

| panel | label | bbox (x,y,w,h) | conf | flags |
|---|---|---|---|---|
| p01 | M11 | 0,0,686,424 | 1.00 | seeded_from_annotations |

![panels](overlays/panels.png)

## Calibration

| panel | unit | scale | slope (unit/px) | r2 | ticks used | flags |
|---|---|---|---|---|---|---|
| p01 | mm | linear | -0.010526 | 1.0000 | 2/2 | seeded_from_annotations |

## Extraction (lineformer_local)

| panel | candidates | selected | conf | coverage | s_alpha | method |
|---|---|---|---|---|---|---|
| p01 | 97 | 5 | 0.065 | 1.000 | 0.710 | gemini_pick |

## Series

### p01
- csv: `series/p01.csv` (30 samples, 0 gaps, baseline off)
- confidence chain: panel=1.00, calibration=1.00, extraction=0.06, coverage=1.00
- QC: **ok** The extracted curve follows the original data line closely with negligible deviations well within one grid unit.

![curve](overlays/curve_p01.png)
![series](overlays/reconstruction_p01.png)
